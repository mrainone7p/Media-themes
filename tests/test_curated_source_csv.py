...
import shared.curated_source_csv as curated_source_csv
import web.logic as logic


class CuratedSourceSharedParserTests(unittest.TestCase):
    ...
    def test_shared_parser_normalizes_headers_offsets_and_legacy_fields(self):
        rows = curated_source_csv.parse_curated_source_csv_rows(self.SAMPLE_CSV)
    ...
    def test_web_parser_keeps_blank_source_rows_for_import_summary(self):
        rows = logic.parse_curated_source_csv(self.SAMPLE_CSV)
    ...
    def test_worker_parser_requires_source_urls_for_matching_catalog(self):
        rows = media_tracks._parse_curated_source_text(self.SAMPLE_CSV)
