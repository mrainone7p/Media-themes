...
def _broadcast(msg):
    ...
    For @@PROGRESS@@ events: if queue is full, drain the oldest item and insert the
    new one so clients never get disconnected due to fast progress streams.
    For critical events (__DONE__, log lines): always try to deliver.
    """
    ...
    for q in _run_clients:
        try:
            ...
            if is_progress:
                # Drop oldest item (stale progress) and insert the fresh one
                try:
                    q.get_nowait()
                    q.put_nowait(msg)
                except Exception:
                    pass  # skip this progress tick — next one will arrive shortly
            else:
                # For non-progress events try one drain then re-insert
                try:
                    q.get_nowait()
                    q.put_nowait(msg)
                except Exception:
...
def _fetch_golden_source_catalog(url):
    """Fetch and parse the Golden Source CSV. Expected columns include tmdb_id, source_url."""
...
def theme_info():
    ...
    return jsonify({"duration":dur,"size":size,"size_kb":round(size/1024,1)})
...
def delete_theme():
    ...
    # 1. Match by rating_key in the specified library
    if key:
        row = next((r for r in rows if str(r.get("rating_key","") or "").strip() == key), None)

    # 2. Match by folder in the specified library
    if not row and folder_hint:
        row = next((r for r in rows if str(r.get("folder","") or "").strip() == folder_hint), None)

    # 3. Cross-library search — walk all enabled libraries
    if not row:
        cfg_libs = load_config().get("libraries", [])
        for lib_entry in cfg_libs:
            lib_name = lib_entry.get("name","")
            if not lib_name or lib_name == lib:
                continue
            ...
            if row:
                # Switch to the library that actually owns this row
                path_ledger = alt_path
                rows = alt_rows
                break

    # ── Determine folder to operate on ───────────────────────────────────────
    # If still no ledger row, fall back to folder_hint directly (file-only delete)
    ...
    if not theme_path.exists():
        # File already gone — update ledger if we have the row
        if row:
            row["status"] = "PENDING"
            row["last_updated"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    try:
        ...
        if row:
            ...
            # File deleted but no ledger row found — still a success
...
def run_stream():
    # Queue large enough for libraries with thousands of items without overflow.
    # _broadcast handles full queues gracefully for progress events anyway.
