 ...
 */
const TABLE_COLUMNS=[
  {id:'select',label:'Select',width:'56px',minWidth:'56px',align:'center',sortable:false,renderHeader:()=>'<input type="checkbox" class="row-cb" id="db-select-all" onchange="toggleAllSelect(this)" title="Select all filtered">',renderCell:(row)=>SelectCell(row)},
...
// ── Theme toggle ─────────────────────────────────────────────────────────────
function _syncThemeToggleButtons(nextTheme){
...
function toggleTheme(){
  ...
  _syncThemeToggleButtons(nextTheme);
  try{localStorage.setItem('mt-theme',nextTheme);}catch(e){}
}
(function(){
  try{
    ...
    setTimeout(()=>{
      const appliedTheme=document.documentElement.getAttribute('data-theme')==='light'?'light':'dark';
      _syncThemeToggleButtons(appliedTheme);
    },0);
  }catch(e){}
})();
...
async function openSearchModal(rk,title,year,lib){
  ...
  if(curatedDesc) curatedDesc.textContent=hasCurated?'Use the curated source already linked to this item.':'';
  if(curatedOffset){
    curatedOffset.textContent=hasCurated?`Offset ${curatedOffsetValue}`:'';
    curatedOffset.classList.toggle('hidden', !hasCurated);
  }
  if(curatedMeta){
    curatedMeta.textContent=hasCurated?`Curated match • starts at ${curatedOffsetValue}`:'';
    curatedMeta.classList.toggle('hidden', !hasCurated);
  }
